...
def registrar_analisis(prob, factores, label, ccaa, sector, contrato, ratio, n_sesion=1):
    fila = {
        ...
        "ratio": ratio,
        "n_sesion": n_sesion,
        "abuso": "SI" if n_sesion > 10 else "NO"
    }
...
if not st.session_state.captcha_ok:
    ...
    <div style="max-width:360px;margin:15vh auto;text-align:center;">
        <div style="font-family:'DM Serif Display',serif;font-size:1.6rem;
                    color:#0f172a;margin-bottom:8px;">Predice_Impago</div>
    ...
    with col_c:
        ...
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;
            ...
            <div style="font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;
                        color:#94a3b8;margin-bottom:12px;">¿Cuánto es?</div>
            <div style="font-family:'DM Serif Display',serif;font-size:2.8rem;
                        color:#0f172a;font-weight:700;margin-bottom:20px;">
                {st.session_state.captcha_q}
            </div>
        </div>
        """, unsafe_allow_html=True)

        respuesta = st.number_input("Tu respuesta", min_value=-99, max_value=999,
                                    step=1, key="captcha_input",
                                    label_visibility="collapsed")

        if st.button("Continuar →", key="captcha_btn"):
            if int(respuesta) == st.session_state.captcha_r:
...
if st.button("Analizar perfil →", disabled=(_tiempo_restante > 0 or _limite_alcanzado)):
    if nomina <= 0:
        ...
        registrar_analisis(prob, factores, label, ccaa, sector, contrato, round(alquiler/nomina*100) if nomina > 0 else 0, st.session_state.n_analisis)
...
# ── BANNER PUBLICITARIO (antes del footer) ─────────────────────────────────
    st.markdown("""
...
# ── Email + PDF ────────────────────────────────────────────────────────────    # ── Email + PDF ────────────────────────────────────────────────────────────
    ...
    st.markdown("""<div style="text-align:center;margin-bottom:20px;">
        <div style="font-family:'DM Serif Display',serif;font-size:1.4rem;color:#0f172a;margin-bottom:6px;">
            Descarga gratis tu informe completo
        </div>
        <div style="font-size:0.88rem;color:#475569;max-width:420px;margin:0 auto;line-height:1.6;">
            ...
            calculadora de coste real, cláusulas contractuales recomendadas
            y comparativa con el perfil medio español.
        </div>
    </div>""", unsafe_allow_html=True)
