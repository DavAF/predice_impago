...
def calcular_score(edad, nomina, alquiler, contrato, antiguedad,
    ...
    score += RIESGO_PERSONAS.get(min(personas, 5), 0.09) * 0.01
    return round(min(0.90, max(0.02, score)), 4), r
...
def generar_pdf(datos_inquilino, prob, factores, label, conclusion, detalle):
    ...
    if prob < 0.08:   sh = "#16a34a"
    elif prob < 0.18: sh = "#d97706"
    elif prob < 0.32: sh = "#ea580c"
    else:             sh = "#dc2626"
    estilos = getSampleStyleSheet()
    t_s  = ParagraphStyle("t",  parent=estilos["Normal"], fontSize=22, textColor=COLOR_TEXTO, spaceAfter=4, fontName="Helvetica-Bold")
    su_s = ParagraphStyle("su", parent=estilos["Normal"], fontSize=10, textColor=COLOR_GRIS, spaceAfter=2, fontName="Helvetica")
    se_s = ParagraphStyle("se", parent=estilos["Normal"], fontSize=9, textColor=COLOR_ACENTO, spaceAfter=4, fontName="Helvetica-Bold")
    b_s  = ParagraphStyle("b",  parent=estilos["Normal"], fontSize=10, textColor=COLOR_TEXTO, spaceAfter=6, fontName="Helvetica", leading=14)
    d_s  = ParagraphStyle("d",  parent=estilos["Normal"], fontSize=8, textColor=COLOR_GRIS, spaceAfter=4, fontName="Helvetica", leading=11)
    story = []
    ...
    story.append(Paragraph("Resultado", se_s))
    res = [[
        Paragraph(f"<font size=32 color='{sh}'><b>{prob_pct}%</b></font>", estilos["Normal"]),
        Paragraph(f"<font size=14 color='{sh}'><b>{label}</b></font><br/><br/><font size=10 color='#1e293b'>{conclusion}</font>", estilos["Normal"])
    ]]
    rt = Table(res, colWidths=[40*mm, 130*mm])
    rt.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"MIDDLE"),("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#f8fafc")),
        ("TOPPADDING",(0,0),(-1,-1),12),("BOTTOMPADDING",(0,0),(-1,-1),12),("LEFTPADDING",(0,0),(-1,-1),12)]))
    story.append(rt)
    story.append(Spacer(1,8))
    story.append(Paragraph(f"<b>Detalle:</b> {detalle}", b_s))
    ...
    filas = [["Variable","Valor"],
        ...
        ["Ratio esfuerzo", f"{round(datos_inquilino['alquiler']/max(datos_inquilino['nomina'],1)*100)}%"],
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#e2e8f0"), spaceAfter=10))
    story.append(Paragraph("Factores vs Perfil Medio España", se_s))
    ff = [["Factor","Inquilino","Perfil medio","Diferencia"]]
    for nombre, val in factores.items():
        med = PERFIL_MEDIO.get(nombre, 0.10)
        diff = int(val*100) - int(med*100)
        signo = f"+{diff}" if diff > 0 else str(diff)
        ff.append([nombre, f"{int(val*100)}%", f"{int(med*100)}%", signo+"%"])
    ft = Table(ff, colWidths=[75*mm, 30*mm, 35*mm, 30*mm])
    ft.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#0f172a")),
        ("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
        ("FONTSIZE",(0,0),(-1,-1),9),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f8fafc")]),
        ("GRID",(0,0),(-1,-1),0.3,colors.HexColor("#e2e8f0")),
        ("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),("LEFTPADDING",(0,0),(-1,-1),8)]))
    ...
    story.append(Paragraph(
        "AVISO LEGAL: Informe orientativo basado en estadísticas públicas INE EPA Q4 2025, "
        "Banco de España, Fundación Alquiler Seguro 2023 y SEPE. "
...
with col_c: edad    = st.selectbox("Franja de edad", list(PARO_POR_EDAD.keys()), index=2)
with col_d: personas= st.number_input("Personas en el piso", min_value=1, max_value=8, value=1, step=1)
...
if nomina > 0:
    ...
    cr = "#16a34a" if ratio_live<=30 else "#d97706" if ratio_live<=35 else "#dc2626"
    icono = "✓ óptimo" if ratio_live<=30 else "⚠ límite BdE" if ratio_live<=35 else "✗ excede umbral crítico"
...
if st.button("Analizar perfil →"):
    if nomina <= 0:
        ...
        prob, factores = calcular_score(edad, nomina, alquiler, contrato, antiguedad,
                                        fuente, impagos, sector, ccaa, nacionalidad, personas)
...
if "resultado" in st.session_state:
    ...
    with tab1:
        ...
        st.markdown(f'<div style="display:flex;justify-content:center;">{svg_radar}</div>',
                    unsafe_allow_html=True)
    ...
    if email and acepta_comunicaciones:
        ...
        pdf_buffer = generar_pdf(r["datos"], prob, factores, label, conclusion, detalle)
